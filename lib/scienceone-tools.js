...
function scienceOneToolStatus() {
  return {
    crystal_structure: {
      ready: true,
      upstream: `${SCIENCEONE_RUN_BASE}/crystalStructureGen`
    },
    quantum_espresso: {
      ready: true,
      upstream: `${SCIENCEONE_RUN_BASE}/QuantumEspresso`
    },
    band_structure: {
      ready: Boolean(String(process.env.SCIENCEONE_BAND_STRUCTURE_URL || "").trim()),
      upstream: String(process.env.SCIENCEONE_BAND_STRUCTURE_URL || "").trim() || null,
      note: String(process.env.SCIENCEONE_BAND_STRUCTURE_URL || "").trim()
        ? null
        : "ScienceOne documents this tool as conversation-only; configure its direct request URL when ScienceOne provides one."
    }
  };
}

async function runScienceOneTool(tool, input, options = {}) {
  ...
  try {
    ...
    const authToken = String(options.authToken || process.env.SCIENCEONE_API_TOKEN || "").trim();
