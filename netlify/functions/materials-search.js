...
function response(statusCode, payload, origin) {
  ...
  return { statusCode, headers, body: payload ? JSON.stringify(payload) : "" };
}
...
exports.handler = async function handler(event) {
  ...
  if (method === "OPTIONS") {
    if (origin && !isAllowedOrigin(origin, process.env)) return response(403, { ok: false, error: "Origin is not allowed." }, origin);
